...
# If you're using HAADJ controlled validation via client side registry keys, uncomment these sections, adding registry items for Hybrid Azure Active Directory Join 
#Write-Host "Setting registry items for Hybrid Azure Active Directory Join"
#reg.exe add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\CDJ\AAD" /v TenantId /t REG_SZ /d TenantID /f | Out-Host
#reg.exe add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\CDJ\AAD" /v TenantName /t REG_SZ /d TenantName /f | Out-Host

# Adding Hybrid Azure AD Join script process
